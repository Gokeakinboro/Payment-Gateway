...
const CARD = { number: '5199111186151739', month: '05', year: '27', cvv: '819', name: 'Akinboro May' };
const CURRENCY = 'NGN';
const AMOUNT   = '100.00';
...
function mpgsReq(orderId) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({
      apiOperation: 'PAY',
      order: { amount: AMOUNT, currency: CURRENCY, description: 'UBA 3DS auto-test' },
      ...
      transaction: { reference: 'UBA-AUTO-' + Date.now() },
      customer: { email: 'mayakinboro15@gmail.com', firstName: 'May', lastName: 'Akinboro' },
    });
    const options = {
      hostname: HOST,
...
(async () => {
  const orderId = 'UBA-' + CARD.number.slice(-4) + '-' + Date.now();
  console.log('[uba-autotest] started — order:', orderId);

  let subject, html;
  try {
    const { status, body: b } = await mpgsReq(orderId);
    console.log('[uba-autotest] HTTP', status, 'result:', b.result);
    ...
    if (b.result === 'PENDING_AUTHENTICATION') {
      ...
      html = [
        ...
        '<p>Please click the link below to verify your card payment of <strong>NGN 100.00</strong>:</p>',
  ...
  try {
    await transporter.sendMail({
      ...
      to:  'mayakinboro15@gmail.com',
      cc:  'gokeakinboro@gmail.com',
      subject,
      html,
    });
    console.log('[uba-autotest] email sent to mayakinboro15@gmail.com, cc gokeakinboro@gmail.com');
  } catch (e) {
    console.error('[uba-autotest] email failed:', e.message);
  }
  process.exit(0);
})();
