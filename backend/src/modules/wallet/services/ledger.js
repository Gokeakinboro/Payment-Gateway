...
async function moveWallet(tx, { walletId, direction, amount, type, maxBalance, departmentId, transactionId, counterparty, note, createdBy, approvedBy, reference }) {
  ...
  const rows = await tx.$queryRawUnsafe(
    `SELECT id::text, merchant_id::text AS merchant_id, member_id::text AS member_id,
            balance::text AS balance, status FROM mw_wallets WHERE id = $1::uuid FOR UPDATE`, walletId);
  ...
  await tx.$executeRawUnsafe(`UPDATE mw_wallets SET balance = $1, updated_at = now() WHERE id = $2::uuid`, newBal, walletId);
...
async function moveDepartment(tx, { merchantId, departmentId, direction, amount, type, memberId, walletLedgerId, reference }) {
  ...
  const last = await tx.$queryRawUnsafe(
    `SELECT balance_after::text AS b FROM mw_dept_ledger WHERE department_id = $1::uuid ORDER BY created_at DESC, id DESC LIMIT 1`, departmentId);
...
async function reconcile(merchantId) {
  const r = await prisma.$queryRawUnsafe(
    `SELECT (SELECT COALESCE(SUM(balance),0) FROM mw_wallets WHERE merchant_id=$1::uuid)::text AS wallet_float,
            (SELECT COALESCE(SUM(CASE WHEN direction='credit' THEN amount ELSE -amount END),0)
               FROM mw_ledger WHERE merchant_id=$1::uuid)::text AS ledger_net`, merchantId);
